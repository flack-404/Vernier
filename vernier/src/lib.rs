//! Vernier — a bounded-correction scoring module for Telegraph Protocol.
//!
//! A fork of `telegraph-wasm-baseline`. The four signals, their weights, the
//! MiniLM path and the embedded weight blob are byte-identical to it, because
//! that identity is what earns rank agreement with the incumbent scorer — the
//! check that rejected three of the five scripts submitted before this one.
//!
//! On top of that, one thing is added: a correction layer capped at 0.12 that
//! fires only where the baseline is provably blind. The baseline compares meaning
//! rather than content, so it cannot distinguish `$94.06` from `$94,060`, and
//! roughly half of this network's traffic is prices, balances, addresses and
//! identifiers. See `corrections.rs` for the derivation of the cap and the
//! evidence behind each detector.
//!
//! Build with `--features real_weights`. Building `--no-default-features`
//! disables the correction layer and yields a module behaviourally identical to
//! the baseline, which is how `tg-score verify` proves the fork left it intact.
//!
...
//! | `correction_answer` | `(i32,i32,i32,i32,i32,i32) → i32` | Correction-layer detail; returns ptr to f32[6] |
...
// `no_std` for the WASM target; under `cargo test` the host target links `std` so
// the correction layer's unit tests can run natively. See allocator.rs.
...
// ── Static output buffer for correction_answer() ─────────────────────────────
// 6 values × 4 bytes = 24 bytes.
// Layout: [total, numeric, identifier, refusal, gt_refused, numeric_coverage]
#[cfg(feature = "corrections")]
const CORRECTION_DIM: usize = 6;
#[cfg(feature = "corrections")]
static mut CORRECTION_BUF: [f32; CORRECTION_DIM] = [0f32; CORRECTION_DIM];
...
/// Apply the bounded correction to a baseline composite.
///
/// Kept in one place so `rank_answer`, `rank_answer_cached` and
/// `breakdown_answer` can never drift apart in what they report — the same
/// reason the baseline defines `composite()` once.
///
/// Compiled out entirely without the `corrections` feature, leaving the
/// baseline's own composite as the returned score.
#[inline]
fn corrected(_question: &str, _ground_truth: &str, _miner_answer: &str, base: f32) -> f32 {
    #[cfg(feature = "corrections")]
    {
        let c = corrections::analyse(_question, _ground_truth, _miner_answer);
        return math::clamp01(base + c.total);
    }
    #[cfg(not(feature = "corrections"))]
    {
        base
    }
}
...
pub unsafe extern "C" fn rank_answer(
    ...
    corrected(question, ground_truth, miner_answer, base)
}
...
pub unsafe extern "C" fn rank_answer_cached(
    ...
    let base = composite(relevance, correctness, lexical, len_quality);
    // The question text is not available on this path — only its cached vector —
    // and the correction layer does not read the question, so passing an empty
    // string here is exact rather than approximate. Should a future detector need
    // the question, this path must take its text too.
    corrected("", ground_truth, miner_answer, base)
}
...
pub unsafe extern "C" fn breakdown_answer(
    ...
    // IDX_COMPOSITE carries the CORRECTED score, so that breakdown_answer and
    // rank_answer always agree on the final number. The four signal slots remain
    // the baseline's raw signals, so they will not sum to the composite whenever
    // the correction fires; `correction_answer` reports the difference.
    let composite_score = corrected(
        question,
        ground_truth,
        miner_answer,
...
/// Correction-layer detail for one triple.
///
/// Writes six float32s into `CORRECTION_BUF` and returns its byte offset, so a
/// host can read 6 × 4 = 24 bytes from that address:
///
/// ```text
///   [0] total              the applied adjustment, <= 0
///   [1] numeric            numeric detector contribution
///   [2] identifier         identifier detector contribution
///   [3] refusal            refusal detector contribution
///   [4] gt_refused         1.0 when the ground truth was itself a refusal
///   [5] numeric_coverage   matched fraction of headline figures, -1 if unchecked
/// ```
///
/// A scoring module is otherwise opaque: one f32 comes back with no account of
/// how it was reached. This export is what lets `tg-score verify` check this
/// module against its Go reference implementation row by row, and what lets
/// anyone else see why a score moved.
///
/// Returns 0 when built without the `corrections` feature.
#[no_mangle]
pub unsafe extern "C" fn correction_answer(
    q_ptr: i32,  q_len: i32,
    ...
    #[cfg(feature = "corrections")]
    {
        ...
        let c = corrections::analyse(question, ground_truth, miner_answer);
        CORRECTION_BUF[0] = c.total;
        CORRECTION_BUF[1] = c.numeric;
        CORRECTION_BUF[2] = c.ident;
        CORRECTION_BUF[3] = c.refusal;
        CORRECTION_BUF[4] = if c.gt_refused { 1.0 } else { 0.0 };
        CORRECTION_BUF[5] = c.numeric_coverage;
        CORRECTION_BUF.as_ptr() as i32
    }
    #[cfg(not(feature = "corrections"))]
    {
        let _ = (q_ptr, q_len, gt_ptr, gt_len, ma_ptr, ma_len);
        0
    }
}
